export default function BlastSection() {
  return (
    <div className="uppercase max-w-[60rem]  text-center mx-auto py-36 text-white">
      <h3 className="text-6xl border-t-4 pt-11 font-semibold">
        Blast your projects with our All-in-one tool
      </h3>
      <p className="mt-7 text-xl">
        Introducing the most comprehensive DEX on BLAST! {"We've"} built a
        user-friendly platform that empowers anyone to launch their projects on
        this blockchain without the need for programming skills. Your project,
        your way <br /> – unleash your creativity effortlessly with our
        all-inclusive DEX.
      </p>
    </div>
  );
}
